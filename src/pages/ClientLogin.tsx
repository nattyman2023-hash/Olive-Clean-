...
export default function ClientLogin() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!existingClient) {
      setLoading(false);
      toast.error("No account found for this email. Please contact Olive Clean to get started.");
      return;
    }
          ...
          {mode === "login" && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex justify-between text-xs">
                ...
                <button type="button" onClick={() => setMode("signup")} className="text-primary font-medium hover:text-primary/80 transition-colors">
                  Create account
                </button>
              </div>
            </form>
          ...
          {mode === "signup" && (
            <form onSubmit={handleSignUp} className="space-y-4">
              <p className="text-xs text-muted-foreground bg-muted rounded-lg p-3">
                Sign up is available to existing Olive Clean clients. Use the email address on file with us.
              </p>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">Email</label>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required className="rounded-xl" />
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">Password</label>
                <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" required minLength={6} className="rounded-xl" />
              </div>
              <Button type="submit" disabled={loading} className="w-full rounded-full active:scale-[0.97] transition-transform">
                {loading && <Loader2 className="h-4 w-4 animate-spin mr-2" />}
                Create Account
